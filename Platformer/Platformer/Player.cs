...
namespace Platformer
{
    ...
    class Player
    {
        ...
        public Rectangle BoundingRectangle
        {
            get
            {
                //Change bounding boxes if possessing a sushi
                if (possession == PossessionStatus.Sushi)
                {
                    int left = (int)Math.Round(Position.X - sprite.Origin.X) + localBounds.X;
                    int top = (int)Math.Round(Position.Y - sprite.Origin.Y) + localBounds.Y;

                    return new Rectangle(left+localBounds.Width/2, top+localBounds.Height/2, localBounds.Width/2, localBounds.Height/2);
                }
                else
                {
                    int left = (int)Math.Round(Position.X - sprite.Origin.X) + localBounds.X;
        ...
        public void LoadContent()
        {
            ...
            idleAnimation = new Animation(Level.Content.Load<Texture2D>("Sprites/Player/Idle"), 0.15f, true);
            runAnimation = new Animation(Level.Content.Load<Texture2D>("Sprites/Player/Run"), 0.15f, true);
            jumpAnimation = new Animation(Level.Content.Load<Texture2D>("Sprites/Player/Jump"), 0.15f, false);
        ...
        public void ApplyPhysics(GameTime gameTime)
        {
            ...
            if (possession == PossessionStatus.Lantern)
            {
                velocity.Y = DoubleJump(velocity.Y, gameTime);
            }
            else 
            {
        ...
        /// <summary>
        /// Highlights the nearest enemy for clarity.
        /// </summary>
        private void HighlightEnemy()
        {
            ...
            foreach(Enemy e in enemies)
            {
                double distance = Math.Pow(position.X - e.Position.X, 2.0) + Math.Pow(position.Y - e.Position.Y, 2.0);
            ...
            {
                double distance = Math.Pow(position.X - e.Position.X, 2.0) + Math.Pow(position.Y - e.Position.Y, 2.0);
                if (distance == temp && !e.possessed && distance < 20000.0) e.IsNearest = true;
                else e.IsNearest = false;
            }
        }

        private float DoubleJump(float velocityY, GameTime gameTime)
        {
            // If the player wants to jump
            if (isJumping)
            {
